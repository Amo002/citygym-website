...
export function Contact({ brand }: { brand: SiteContent["brand"] }) {
  ...
  const contactRows = [
    { icon: Phone, label: "Men's Club", value: brand.phoneMen, href: `tel:${brand.phoneMen}` },
    { icon: Phone, label: "Women's Club", value: brand.phoneWomen, href: `tel:${brand.phoneWomen}` },
    { icon: Mail, label: "Email", value: brand.email, href: `mailto:${brand.email}` },
    { icon: MapPin, label: "Location", value: brand.address, href: "#location" },
  ];
  ...
  return (
    <section id="contact" className="mx-auto max-w-7xl px-5 py-24 sm:px-8 sm:py-32">
      <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
        <div>
          <SectionHeading eyebrow="Get In Touch" heading="Let's start your journey" />
          <Reveal i={2}>
            <p className="mt-6 max-w-md text-lg text-fg-muted">
              Drop us a message or call directly. Our team will help you pick the perfect plan and
              book your first session.
            </p>
          </Reveal>
          <div className="mt-10 space-y-4">
            {contactRows.map((r, i) => (
              <Reveal i={i} key={r.label}>
          ...
          <form onSubmit={submit} className="card p-7 sm:p-9">
            {status === "done" ? (
              <div className="flex flex-col items-center gap-4 py-16 text-center">
                ...
                <p className="text-fg-muted">We&apos;ll be in touch within 24 hours.</p>
                <button
                  type="button"
              ...
              <div className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <input className="field" placeholder="Full name" required value={form.name} onChange={(e) => set("name", e.target.value)} />
                ...
                <textarea className="field min-h-32 resize-none" placeholder="Tell us your goals..." required value={form.message} onChange={(e) => set("message", e.target.value)} />
                {status === "error" && <p className="text-sm font-medium text-red-500">Something went wrong. Try again.</p>}
                <button
                  type="submit"
